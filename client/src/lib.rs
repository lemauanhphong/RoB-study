...
// BUG: find a way to call encrypt with key from global or start function in order not to generate
// key each time
// #[wasm_bindgen]
pub fn encrypt(cipher: Aes256Gcm, plaintext: Vec<u8>) -> Vec<u8> {
...
async fn send_machine_info(machine_id: String, key: Vec<u8>) {
    let rsa_public_key = RsaPublicKey::from_pkcs1_pem(PUBLIC_KEY).unwrap();

    let encrypted_key = rsa_public_key
        .encrypt(&mut OsRng, Pkcs1v15Encrypt, key.as_ref())
        .unwrap();
...
async fn start() {
    ...
    send_machine_info(machine_id, key.to_vec()).await;
